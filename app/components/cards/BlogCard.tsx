...
// Define dimensions for each layout type
const imageDimensions: Record<
  NonNullable<BlogCardProps["layout"]>,
  { width: number; height: number }
> = {
  vertical: { width: 600, height: 400 },
  horizontal: { width: 80, height: 80 },
  featured: { width: 1200, height: 500 },
};
  ...
  const imageUrl =
    (post.mainImage &&
      urlFor(post.mainImage)
        .width(imageDimensions[layout].width)
        .height(imageDimensions[layout].height)
        .url()) ||
    ...
    <article className={`${baseStyles} ${layoutStyles[layout]} ${className}`}>
      <Link href={`/blog/${post.slug.current}`}>
        ...
        >
          ...
          {/* Title */}
          <h3
            className={`font-bold text-gray-900 group-hover:text-amber-600 transition-colors mb-3 ${
          ...
          >
            ...
            <div className={showAuthor ? "" : "w-full"}>
              <Link
                ...
                className={`font-semibold hover:text-amber-700 transition-colors inline-flex items-center gap-2 ${
                  layout === "featured"
